...
def main():
    ...
    print(f"   From: {MERCHANT_ADDRESS} (via token account)")
    print(f"   To: {NEW_RECIPIENT_ADDRESS} (NEW TEST RECIPIENT)")
    ...
    credential = generate_mock_credential(NEW_RECIPIENT_ADDRESS, amount)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'signature' in result:
                ...
                print(f"   • Customer identity: HIDDEN from recipient")
                print(f"   • Payment amount: {amount} tokens")
                print(f"   • Transaction ID: {tx_sig}")
                print(f"   • Recipient: {NEW_RECIPIENT_ADDRESS[:16]}...")
                print(f"   • Recipient sees: Relayer address only")
